...
describe('Select', function () {
  ...
  it('partial build', function () {
    ...
    expect(test.build()).to.be.equal(`SELECT * FROM user WHERE age=20 LIMIT 10 OFFSET 20 ORDER BY name GROUP BY name;`);
  });
  ...
  it('full', function () {
    ...
    expect(test).to.be.equal(`SELECT * FROM user WHERE age=20 LIMIT 10 OFFSET 20 ORDER BY name GROUP BY name;`);
  });
});
